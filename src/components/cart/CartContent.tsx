            ...
            {cart.map((item: Cart) => (
                <div key={item.product.id} className="flex py-2 justify-between border mb-4 p-4">
                    <div className='flex items-center justify-center w-[150px]' >
                        <Link href={`/product/${item.product.slug}`} className='w-full ' prefetch={true}>
                            <Image src={`${item.product.image}?profile=basic&w=150`} alt={item.product.name}
                                width={150}
                                height={150}
                    ...
                    <div className="flex flex-col justify-between w-2/3 px-2">
                        ...
                        <div className='flx flex-col gap-2'>
                            <span className='text-xs text-green-600'>Stock: {item.product.stock}</span>
                            <div>
                                <button
                    ...
                    <div className='flex flex-col justify-between items-end text-right w-max'>
                        ...
                        <div className=''>
                            <span className='text-sm'>Total: ${item.product.price * item.quantity}</span>
                        </div>
                    </div>
                </div>
