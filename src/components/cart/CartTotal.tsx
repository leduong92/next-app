    ...
    return (
        <div className='flex flex-col justify-between gap-2'>
            ...
            {voucher && (!voucher.minOrder || subtotal >= voucher.minOrder) && (
                <div className="flex justify-between">
                    <span className='text-green-600 text-sm'>Voucher ({voucher.code} - {voucher.value}%):</span>
                    <span className='text-sm'>-{voucher.type === "percentage" ? `$${discountAmount}` : `${voucher.value}`}</span>
                </div>
            )}
            <div className='flex justify-between'>
